...
Once the human has reviewed and approved `documents/pull-request.md`, raise the actual GitHub PR using the `gh` CLI:
...
### Step 2: Create the PR using the pull-request.md as body

```bash
gh pr create \
  --title "<title from pull-request.md Metadata section>" \
  --body-file "documents/pull-request.md" \
  --base main \
  --head <current-branch>
```

### Step 3: Create required labels if they do not exist

```bash
gh label create "ai-generated" --color "0075ca" --description "Generated by AI" 2>/dev/null || true
gh label create "claude" --color "7057ff" --description "Generated by Claude" 2>/dev/null || true
```

### Step 4: Add labels to the PR

```bash
gh pr edit <pr-number> --add-label "ai-generated" --add-label "claude"
```

Required labels (from Metadata Requirements):
- `ai-generated`
- `claude`

Labels must be applied to the PR's Labels field — not written into the PR body/description.

### Notes

- Use `--body-file` (not `--body`) to avoid shell quoting issues with markdown content.
- If `gh` is not installed, instruct the human to install it via `winget install GitHub.cli` and authenticate with `gh auth login`.
- Report the final PR URL to the human on completion.
